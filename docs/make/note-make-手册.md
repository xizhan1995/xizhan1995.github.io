...
本手册使用一个示例程序作为书中例子的操作对象，这个程序是一个文本编辑器，它由8个源代码
文件和3个头文件构成。
...
直接输入make，不带任何参数，它执行的就是 default goal。
...
哪种风格好，看个人喜好。这种风格更紧凑，但第一种风格把目标的所有依赖都列在一处，所以更清晰。
## 2.7 清理目录的规则
...
$(myrule)
````
实际等效于 `target: ; echo built`，其中的换行符仍旧保留，但效果和空白字符相同。
...
targets 是文件名列表，空格做分隔符。文件名中可以使用通配符（参见[Using Wildcard Characters in File Names][]）。
...
windows 和 dos 系统上，路径分隔符是反斜杠，在这些系统行，make 同时支持两种风格的路径分隔符，如
...
patern 使用 % 作为特殊字符，它匹配任意长度的字符序列（包括零个）。反斜杠转义可以取消
其特殊含义（\%）。
...
比如，要用到 curses 库，不是指定 libcurses.so，而是以 -lcurses 的形式指定依赖，make会知道
...
- .ONESHELL         只要定义了这个 target。所有的规则都使用单进程，也就是
...
首先，recipe 中的变量引用和函数引用语法与其它地方相同，都是 $(xxx ) 的形式。其次，
...
当设置了 `.ONESHELL:`，会检查 recipe 的第一行（且仅检查第一行），并在把recipe传递给
...
同时，recipe 中继承的环境变量 SHELL，不是 makefile 的SHELL变量，而是真实的用户变量
SHELL。
...
当某个 recipe 的某个命令执行失败，且没有忽略此行命令的错误（-i或者-字符），表明此 recipe
...
为了方便，GNU make 工具会设置一个变量 CURDIR，它对应 make 的工作目录（处理完 -C 选项之后）。
...
MAKEFILES 变量会自动层层向内传递，所以可以通过设置此变量（环境变量或make变量）让所有
...
自然，当你编写makefile的时候，你知道自己希望哪条规则生效，你知道make会选择这条规则，
因为你知道这条规则的前置条件预期会存在。
...
对于没有 recipe 的 target 使用隐式规则，可以写一条规则，为该target提供一个空recipe。
ps:空recipe就是只有一个分号的recipe，如：
```makefile
foo:
    ;
```
...
内置隐式规则总是有的，除非明确覆盖或者取消他们。
（-r --no-builtin-rules取消所有内置规则）。
ps：内置规则，都是隐式规则。

不同平台、不同版本的make，其内置的隐式规则不完全一致，在一个不包含makefile的目录下执行
`make -p`命令，可以查看当前make工具实际的内置规则和变量。
本手册列出的内置规则在基于 POSIX 的系统上可用。

即使不指定 -r 选项，也不是所有的内置规则都总是可用的。
在make内部，这里的隐式规则，有不少是通过 suffix rule 实现的，所以它们到时候是否生效，
还要受到 suffix list 的影响，可以通过 .SUFFIXES 这个特殊的target控制 suffix list。
ps：suffix rule 是旧风格，它的继任者就是更强健的模式规则（pattern rule）。
...
- 链接单个对象文件
    ...
    这条规则对于简单情况有效，复杂的情况就要手动编写链接规则。
- 针对C程序的yacc
    %.c: %.y
        $(YACC) $(YFLAGS)
...

通常，你只需要修改上述隐式规则中使用的内置变量即可。

实际上，内置规则实际使用的变量类似于 COMPILE.c, LINK.c，它们保存了上述列出的recpie。
用于编译 .x 源文件的隐式规则使用变量 COMPILE.x，以及用于从 .x 生成可执行文件的内置规则
使用变量 LINK.x，预处理 .x 文件的内置规则使用变量 PREPROCESS.x。

关于生成 .o 文件的内置规则，和OUTPUT_OPTION，以及在某些系统上的tricky。
...
1. 内置规则使用了内置变量，可以在makefile中修改这些内置变量的值，从而定制内置规则的行为。
2. 使用 -R --no-builtin-variables 选项取消所有内置变量的定义
3. 内置规则使用的内置变量可以分成两类；一类指定编译工具，一类指定编译选项。
4. 手册列出了最常用的内置变量。

## 10.4 隐式规则的链式调用
Chains of Implicit Rules

make足够聪明，能够判断出是否需要链式使用隐式规则。比如 .y -> .c -> .o。
...
链式使用隐式规则，提到“中间文件”，它有两个不同于普通文件的地方：
1. 对更新的影响
2. make 用后自动删除
...
中间文件，显式规则的target或依赖不可作为中间文件。除非你要求make这么做。
...
有些优化，比如从 foo.c 生成 foo 可以先生成 .o 再链接成 foo，也可以一步到位，为了效率，
make 会选择一步到位，这种选择通过内置规则的顺序实现的。
...
考虑效率，搜索构建隐式规则的依赖时，不会考虑非尾节点的 "%:" 的隐式规则，因为 % 没有任何普通字符，它
匹配一切东西，匹配的太过宽泛了。
Finally, for performance reasons make will not consider non-terminal match-anything rules (i.e., ‘%:’) when searching for a rule to build a prerequisite of an implicit rule (see Match-Anything Rules).

## 10.5 自定义和重新定义模式规则
Defining and Redefining Pattern Rules

1. 写一条模式规则，就实现了定义一条隐式规则。
2. 隐式规则的target有且仅有一个 %, 它是个通配符，可以匹配任何非空字符串。% 匹配的子串
   叫做枝干（stem）。
3. pattern 中的 % 的展开，是在变量替换和函数执行之后进行的。
...
依赖中的 % 代表同样的枝干。
模式规则生效的前提是：该规则的target能匹配到某个文件，且规则的依赖（将%替换成对应的
子串之后）必须是已存在文件或是可以生成的文件。
...
模式规则甚至可以不书写任何依赖，这时的模式规则其实就是一个万能统配规则，它匹配任何符合
target模式的文件。参考 Last Resort.
...
为解决模式规则中的recipe中，指定动态文件名的问题，make 创建了动态变量。
动态变量仅限于recipe中有效，不要在target和prerequisite中使用（未定义，
会扩展为空串）。
...
目标模式由一个%和可选的前后缀构成，一个模式可以匹配给定的文件名仅当该文件名以模式的前缀
开头，以模式的后缀结尾；匹配成功后，前后缀之间的部分叫做主干（stem）。比如 %.o 匹配
test.o 时，主干就是 test。然后规则的依赖中，%替换为主干，以获得实际的依赖。
比如 `%.o: %.c ;` 匹配 test.o 时，它的依赖就是 test.c。
...
目标模式一般不含斜杆，但同样可以匹配含义斜杠的路径名。
- 匹配的时候只对文件的basename进行匹配，
- 替换依赖中的%的时候，目录会作为前缀，放在结果上。
Q. 目标模式含有斜杠呢？
`e%t: c%r` 匹配文件 src/eat，stem 是 src/a, 对 c%r 替换得到实际依赖是 src/car.
原文如下：
...
仅当匹配它时候，内置规则才会起效。
显式书写的规则优先于内置规则。
注意，如果一个规则 R 的依赖项实际存在，或者有显式描述，那么规则 R 的优先级高于必须借由
链式隐式规则才能生成依赖项的规则。
...
Q. 不能白这个“Note however”这句话的意义？
放在这一段中，好像是说有些情况下隐含规则会优先于明确写出的规则。
可我没能找出这样的例子。
但我找到了另外一个例子：当没有指定任何额外信息的时候，make会从 foo.c 一步编译出 foo，
而当给定规则`foo: foo.o` 的时候，因为显式提到了 foo.o, make 会先编译出 foo.o 再链接
生成 foo。
```bash
[chenx@lcs2 test1]$ ls
bar.c  foo.c  mk1
[chenx@lcs2 test1]$ cat mk1
foo: foo.o
        $(CC) -o $@ $<
[chenx@lcs2 test1]$ make foo
cc     foo.c   -o foo
[chenx@lcs2 test1]$ make bar
cc     bar.c   -o bar
[chenx@lcs2 test1]$ rm foo bar
[chenx@lcs2 test1]$ make -f mk1 foo
cc    -c -o foo.o foo.c
cc -o foo foo.o
[chenx@lcs2 test1]$ make -f mk1 bar
cc     bar.c   -o bar
```

如果多个模式规则都匹配（且对应的依赖都实际存在），那么主干最短的那条规则优先，如果一样长，那么在makefile中的位置
靠前的规则优先。
